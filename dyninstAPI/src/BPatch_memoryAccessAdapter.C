...
{
#if defined(DYNINST_ARCH_ARCH_X86) || defined(DYNINST_ARCH_ARCH_X86_64)
  ...
  assert(nac < 3);
  return bmap;
#elif defined(DYNINST_ARCH_ARCH_POWER)
    ...
    return NULL;
#elif defined(DYNINST_ARCH_ARCH_AARCH64) 
    auto operands = insn.getAllOperands();
    for(std::vector<Operand>::iterator op = operands.begin();
        op != operands.end();
       ++op)
...
void BPatch_memoryAccessAdapter::visit(RegisterAST* r)
{
	...
	unsigned int converted = base.val() & 0xFFFF;
	#if defined(DYNINST_ARCH_ARCH_POWER)
    if((ra == -1) && !setImm) {
        ra = converted;
        return;
    } else if(rb == -1) {
